...
export default function MaterialDetail() {
  ...
  useSeo(
    material
      ...
      : { title: "Материал не найден", description: "" }
  );
  ...
  useJsonLd(
    material
      ? {
          ...
          "@type": "Product",
          name: material.name,
          description: material.heroDesc,
          offers: {
            "@type": "Offer",
            priceCurrency: "RUB",
            price: material.price.replace(/\D/g, ""),
  ...
  if (!material) return <Navigate to="/delivery" replace />;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(SEND_LEAD_URL, {
        ...
        body: JSON.stringify({
          name: "Клиент со страницы товара",
          phone,
          message: `Товар: ${material.name}. Объём: ${tons} т`,
          source: `Страница: ${material.name}`,
        }),
      });
    ...
    <Layout>
      ...
      <section className="bg-brand-black py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-3 mb-6 flex-wrap">
            ...
            <Link to="/delivery" className="text-brand-gray-light text-sm hover:text-brand-orange transition-colors font-golos">Доставка материалов</Link>
            <Icon name="ChevronRight" size={14} className="text-brand-gray-light" />
            <span className="text-brand-orange text-sm font-golos">{material.name}</span>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            <div className="lg:col-span-2">
              <div className="w-16 h-16 bg-brand-orange flex items-center justify-center mb-6">
                <Icon name={material.icon as any} size={30} className="text-white" />
              </div>
              <h1 className="font-oswald text-4xl md:text-6xl text-white uppercase leading-none mb-6">
                {material.name}
              </h1>
              <p className="text-brand-gray-light text-lg leading-relaxed mb-8">{material.heroDesc}</p>
            ...
            <div className="bg-white p-8">
              <div className="font-oswald text-brand-orange text-4xl mb-1">{material.price}</div>
              <div className="text-brand-gray-mid text-sm mb-6 font-golos">{material.priceUnit}</div>
              {!sent ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block font-oswald text-xs tracking-[0.2em] uppercase text-brand-black mb-2">Объём, тонн</label>
                    <input
                      type="number"
                      min={1}
                      value={tons}
                  ...
                  >
                    {loading ? "Отправляем..." : `Заказать ${material.nameGenitive}`}
                  </button>
                </form>
              ) : (
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-10">
            ...
            <h2 className="font-oswald text-3xl md:text-4xl text-brand-black mt-2 orange-line">
              ХАРАКТЕРИСТИКИ {material.name.toUpperCase()}
            </h2>
          </div>
      ...
      <section className="py-20 bg-brand-cream">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            <h2 className="font-oswald text-3xl text-brand-black uppercase mb-6 orange-line pb-3">Где применяется</h2>
      ...
      <section className="py-20 bg-brand-black">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="font-oswald text-4xl md:text-5xl text-white uppercase mb-6">
            Заказать {material.nameGenitive} с доставкой
          </h2>
          <p className="text-brand-gray-light text-lg mb-10 max-w-xl mx-auto">Доставим в день заказа по Ростову-на-Дону, Аксаю и Батайску</p>
