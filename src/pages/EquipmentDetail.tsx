...
import { getEquipmentBySlug } from "@/data/equipmentItems";
...
export default function EquipmentDetail() {
  ...
  useSeo(
    item
      ...
      : { title: "Техника не найдена", description: "" }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(SEND_LEAD_URL, {
        ...
        body: JSON.stringify({
          name: "Клиент со страницы техники",
          phone,
          message: `Техника: ${item.name}. Часов: ${hours}`,
    ...
    <Layout>
      ...
      <section className="bg-brand-black py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-3 mb-6 flex-wrap">
            ...
            <Link to="/equipment" className="text-brand-gray-light text-sm hover:text-brand-orange transition-colors font-golos">Аренда спецтехники</Link>
            <Icon name="ChevronRight" size={14} className="text-brand-gray-light" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="bg-white p-8">
              ...
              <div className="text-brand-gray-mid text-sm mb-6 font-golos">{item.priceUnit}</div>
              {!sent ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block font-oswald text-xs tracking-[0.2em] uppercase text-brand-black mb-2">Часов работы</label>
                    <input
                      type="number"
                      min={4}
                      value={hours}
                  ...
                  >
                    {loading ? "Отправляем..." : "Заказать технику"}
                  </button>
                </form>
              ) : (
      ...
      <section className="py-20 bg-brand-cream">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            <h2 className="font-oswald text-3xl text-brand-black uppercase mb-6 orange-line pb-3">Применение</h2>
      ...
      <section className="py-20 bg-brand-black">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="font-oswald text-4xl md:text-5xl text-white uppercase mb-6">
            Арендовать {item.name.toLowerCase()}
          </h2>
          <p className="text-brand-gray-light text-lg mb-10 max-w-xl mx-auto">Подача техники на объект в течение 2–4 часов</p>
